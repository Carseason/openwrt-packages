...
module("luci.controller.istorego_backend", package.seeall)

local BLOCKSIZE = 2048
local ISTOREGO_PORT = 9910

function index()
    entry({"istorego"}, call("istorego_backend")).leaf=true
end
...
function istorego_backend() 
  local sock = nixio.connect("127.0.0.1", ISTOREGO_PORT) 
  if not sock then
